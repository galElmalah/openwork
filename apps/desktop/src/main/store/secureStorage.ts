 ...
 */
export type ApiKeyProvider = 'anthropic' | 'openai' | 'openrouter' | 'google' | 'xai' | 'deepseek' | 'zai' | 'zai-international' | 'custom' | 'bedrock' | 'litellm' | 'minimax';

/**
 * Get all API keys for all providers
 */
export async function getAllApiKeys(): Promise<Record<ApiKeyProvider, string | null>> {
  const [anthropic, openai, openrouter, google, xai, deepseek, zai, zaiInternational, custom, bedrock, litellm, minimax] = await Promise.all([
    getApiKey('anthropic'),
    getApiKey('openai'),
  ...
  return { anthropic, openai, openrouter, google, xai, deepseek, zai, 'zai-international': zaiInternational, custom, bedrock, litellm, minimax };
}
